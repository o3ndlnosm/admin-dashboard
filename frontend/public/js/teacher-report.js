let currentPage = 1;
let editedReports = JSON.parse(localStorage.getItem("editedTeacherReports")) || [];
let reportsData = [];

// 更新自動上架和狀態
const updateAutoEnableAndStatus = async (reportId, newAutoEnable, newEnable) => {
  try {
    const response = await fetch(`http://localhost:3001/api/teacher-reports/${reportId}/enable`, {
      method: "PATCH",
...
const handleAutoEnableChange = (reportId, newAutoEnable, isCurrentlyUp, now, timeOn) => {
  if (newAutoEnable) {
    ...
    localStorage.setItem("editedTeacherReports", JSON.stringify(editedReports));
  } else {
...
const createTitleCell = (report) => {
  ...
  titleLink.href = `teacher-report-form.html?id=${report.id}`;
...
const createImageCell = (report) => {
  ...
  if (report.image) {
    ...
    img.src = `http://localhost:3001/uploads/teacher-reports/${report.image}`;
...
const pinReport = async (reportId, pinButton) => {
  ...
  try {
    const response = await fetch(`http://localhost:3001/api/teacher-reports/${reportId}/pin`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
const loadReports = async (page = 1) => {
  try {
    const response = await fetch(`http://localhost:3001/api/teacher-reports?page=${page}&forManagement=true`);
    if (!response.ok) {
...
const updateEditedReports = () => {
  const now = new Date();
  editedReports = JSON.parse(localStorage.getItem("editedTeacherReports")) || [];
...
const publishAll = async () => {
  ...
  try {
    const response = await fetch("http://localhost:3001/api/teacher-reports/schedule-all", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    editedReports = [];
    localStorage.removeItem("editedTeacherReports");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const eventSource = new EventSource("http://localhost:3001/api/teacher-reports/events");

  eventSource.onmessage = function (event) {
    const data = JSON.parse(event.data);
    if (data.type === "update-teacher-report" || data.type === "new-teacher-report") {
      loadReports(currentPage);
    }
  };
});
