...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        const dir = 'uploads/teacher-reports/';
...
// 讀取老師報導數據
const readTeacherReports = (callback) => {
    const filePath = path.join(__dirname, '..', 'data', 'teacherReports.json');
...
// 寫入老師報導數據
const writeTeacherReports = (reports, callback) => {
    const filePath = path.join(__dirname, '..', 'data', 'teacherReports.json');
...
cron.schedule('* * * * *', () => { // 每分鐘檢查一次
    readTeacherReports((err, reports) => {
        if (err) {
            console.error('Error reading teacher reports:', err);
            return;
        }
        ...
        reports = reports.map(report => {
            ...
            if (changed) {
                ...
                updated = true;
                notifyClients({ type: 'update-teacher-report', data: report });
            }
            return report;
        });

        if (updated) {
            writeTeacherReports(reports, (err) => {
                if (err) {
                    console.error('Error saving teacher reports:', err);
                } else {
                    console.log('Teacher reports updated based on schedule.');
                }
            });
        }
    });
...
router.post('/', upload.single('image'), (req, res) => {
    ...
    ensureDirectoryExistence(path.join(__dirname, '..', 'data', 'teacherReports.json'));

    readTeacherReports((err, reports) => {
        if (err) {
            return res.status(500).json({ success: false, message: '讀取老師報導數據時出錯' });
        }
        ...
        writeTeacherReports(reports, (err) => {
            if (err) {
                ...
                return res.status(500).json({ success: false, message: '儲存老師報導時出錯' });
            }

            notifyClients({ type: 'new-teacher-report', data: report });

            res.json({ success: true, message: '老師報導已成功提交並儲存為 JSON 檔案！' });
        });
    });
});
...
router.get('/:id', (req, res) => {
    ...
    readTeacherReports((err, reports) => {
        ...
        if (!report) {
            return res.status(404).json({ success: false, message: '老師報導不存在' });
        }
        ...
        writeTeacherReports(reports, (err) => {
            if (err) {
                console.error('Error updating teacher report status:', err);
                return res.status(500).json({ success: false, message: '更新老師報導狀態時出錯' });
            }
...
router.put('/:id', upload.single('image'), (req, res) => {
    ...
    readTeacherReports((err, reports) => {
        ...
        writeTeacherReports(reports, (err) => {
            ...
            notifyClients({ type: 'update-teacher-report', data: updatedReport });

            res.json({ success: true, message: '老師報導已成功更新！' });
        });
    });
});
...
router.patch('/:id/enable', (req, res) => {
    ...
    readTeacherReports((err, reports) => {
        ...
        writeTeacherReports(reports, (err) => {
            if (err) {
                console.error('Error updating teacher report autoEnable status:', err);
                return res.status(500).json({ success: false, message: '更新老師報導自動上架狀態時出錯' });
            }

            notifyClients({ type: 'update-teacher-report', data: reports[reportIndex] });

            res.json({ success: true, message: '老師報導自動上架狀態已更新！' });
        });
    });
});
    ...
    readTeacherReports((err, reports) => {
        ...
        if (updated) {
            writeTeacherReports(reports, (err) => {
                ...
                res.json({ success: true, message: '所有老師報導已排定自動上架！' });
            });
        } else {
            res.json({ success: true, message: '沒有老師報導需要更新。' });
        }
    });
});
...
router.patch('/:id/pin', (req, res) => {
    ...
    readTeacherReports((err, reports) => {
        ...
        writeTeacherReports(reports, (err) => {
            if (err) {
                console.error('Error updating teacher report pinned status:', err);
                return res.status(500).json({ success: false, message: '更新老師報導置頂狀態時出錯' });
            }

            notifyClients({ type: 'update-teacher-report', data: reports[reportIndex] });

            res.json({ success: true, message: '老師報導置頂狀態已更新！' });
        });
    });
});
