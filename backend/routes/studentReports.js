...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        const dir = 'uploads/student-reports/';
...
// 讀取學生報導數據
const readStudentReports = (callback) => {
    const filePath = path.join(__dirname, '..', 'data', 'studentReports.json');
...
// 寫入學生報導數據
const writeStudentReports = (reports, callback) => {
    const filePath = path.join(__dirname, '..', 'data', 'studentReports.json');
...
cron.schedule('* * * * *', () => { // 每分鐘檢查一次
    readStudentReports((err, reports) => {
        if (err) {
            console.error('Error reading student reports:', err);
            return;
        }
        ...
        reports = reports.map(report => {
            ...
            if (changed) {
                ...
                updated = true;
                notifyClients({ type: 'update-student-report', data: report });
            }
            return report;
        });

        if (updated) {
            writeStudentReports(reports, (err) => {
                if (err) {
                    console.error('Error saving student reports:', err);
                } else {
                    console.log('Student reports updated based on schedule.');
                }
            });
        }
    });
...
router.post('/', upload.single('image'), (req, res) => {
    ...
    ensureDirectoryExistence(path.join(__dirname, '..', 'data', 'studentReports.json'));

    readStudentReports((err, reports) => {
        if (err) {
            return res.status(500).json({ success: false, message: '讀取學生報導數據時出錯' });
        }
        ...
        writeStudentReports(reports, (err) => {
            if (err) {
                ...
                return res.status(500).json({ success: false, message: '儲存學生報導時出錯' });
            }

            notifyClients({ type: 'new-student-report', data: report });

            res.json({ success: true, message: '學生報導已成功提交並儲存為 JSON 檔案！' });
        });
    });
});
...
router.get('/:id', (req, res) => {
    ...
    readStudentReports((err, reports) => {
        ...
        if (!report) {
            return res.status(404).json({ success: false, message: '學生報導不存在' });
        }
        ...
        writeStudentReports(reports, (err) => {
            if (err) {
                console.error('Error updating student report status:', err);
                return res.status(500).json({ success: false, message: '更新學生報導狀態時出錯' });
            }
...
router.put('/:id', upload.single('image'), (req, res) => {
    ...
    readStudentReports((err, reports) => {
        ...
        writeStudentReports(reports, (err) => {
            ...
            notifyClients({ type: 'update-student-report', data: updatedReport });

            res.json({ success: true, message: '學生報導已成功更新！' });
        });
    });
});
...
router.patch('/:id/enable', (req, res) => {
    ...
    readStudentReports((err, reports) => {
        ...
        writeStudentReports(reports, (err) => {
            if (err) {
                console.error('Error updating student report autoEnable status:', err);
                return res.status(500).json({ success: false, message: '更新學生報導自動上架狀態時出錯' });
            }

            notifyClients({ type: 'update-student-report', data: reports[reportIndex] });

            res.json({ success: true, message: '學生報導自動上架狀態已更新！' });
        });
    });
});
    ...
    readStudentReports((err, reports) => {
        ...
        if (updated) {
            writeStudentReports(reports, (err) => {
                ...
                res.json({ success: true, message: '所有學生報導已排定自動上架！' });
            });
        } else {
            res.json({ success: true, message: '沒有學生報導需要更新。' });
        }
    });
});
...
router.patch('/:id/pin', (req, res) => {
    ...
    readStudentReports((err, reports) => {
        ...
        writeStudentReports(reports, (err) => {
            if (err) {
                console.error('Error updating student report pinned status:', err);
                return res.status(500).json({ success: false, message: '更新學生報導置頂狀態時出錯' });
            }

            notifyClients({ type: 'update-student-report', data: reports[reportIndex] });

            res.json({ success: true, message: '學生報導置頂狀態已更新！' });
        });
    });
});
